...
export default function HotProduct() {
    ...
    const fetchData = async () => {
        try {
            setLoading(true);
            const response = await fetch("http://tmt020202ccna-001-site1.atempurl.com/api/product-shoes/ds-giay-moi?page="+ `${page}` +"&pageSize=3");
            ...
            <ScrollView horizontal showsHorizontalScrollIndicator={true}>
                <View style={styles.productTabContainer}>
                    {buttons.map((button) => (
                        <TouchableOpacity
                            key={button.id}
                            style={[styles.productTab, activeButton === button.id ? styles.activeProductTab : null]}
                            onPress={() => handlePress(button.id)}
                        >
                            <Text style={[styles.productTabText, activeButton === button.id ? styles.activeProductTabText : null]}>
                                {button.label}
                            </Text>
                        </TouchableOpacity>
                    ))}
                    ...
                    {data.map((item) => (
                        <TouchableOpacity style={styles.productItem} key={item.id}
                            ...
                            <View style={styles.titleProductWrap}>
                                <Text numberOfLines={2} ellipsizeMode="tail" style={styles.titleProduct}>{item.name_product}</Text>
                            </View>
